(() => {
  ...
  const st={sources:[],source:null,kind:"live",packages:[],loadedKinds:new Set(),loadingKinds:new Map(),allPackages:{live:[],vod:[],series:[]},countries:[],assigned:new Map(),packageStreams:new Map(),hiddenCountries:new Set(),activeCountry:null,pendingDeleteCountry:null,loaded:false,loadingCount:0}, $=id=>document.getElementById(id);
  ...
  function visiblePackages(countryId){const rows=st.assigned.get(countryId)||[],ids=new Set(st.allPackages[st.kind].flatMap(x=>x.items.map(streamId)));return rows.filter(x=>[...(st.packageStreams.get(x.id)||[])].some(id=>ids.has(id)))}
