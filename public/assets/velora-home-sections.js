...
function stripChannelPrefixes(value){var original=String(value||"").trim(),name=original;for(var pass=0;pass<64;pass+=1){var prefix=homePrefixes.find(function(candidate){return candidate.length<=name.length&&name.slice(0,candidate.length).toLowerCase()===candidate.toLowerCase()});if(!prefix)break;name=name.slice(prefix.length).trim()}return name||original}
...
function applyHomeChannelRules(section,entries){var rows=Array.isArray(entries)?entries:[];if(!section||section.content_type!=="live")return rows;return rows.map(function(entry){var rawName=String(entry&&entry._veloraHomeRawName||entry&&entry.name||"").trim();if(!rawName||channelNameIsHidden(rawName))return null;return Object.assign({},entry,{name:stripChannelPrefixes(rawName),_veloraHomeRawName:rawName})}).filter(function(entry){return entry&&entry.name})}
