...
function shuffleHomeCards(array){if(!Array.isArray(array)||array.length<=1)return Array.isArray(array)?array.slice():[];var copy=array.slice();for(var i=copy.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var temp=copy[i];copy[i]=copy[j];copy[j]=temp}return copy}
var homeSectionRenderLimit=20;
verifiedEntries=async function(section){var entries=Array.isArray(section.entries)&&section.entries.length?section.entries:(Array.isArray(section.custom_entries)?section.custom_entries:[]);return shuffleHomeCards(entries).slice(0,homeSectionRenderLimit)};
...
window.stripChannelPrefixes=stripChannelPrefixes;
function veloraDetectDarkLogo(imgEl,cardEl){if(!imgEl||imgEl._darkChecked)return;imgEl._darkChecked=true;function runAnalysis(sourceImg){try{var canvas=document.createElement("canvas");var ctx=canvas.getContext("2d",{willReadFrequently:true});if(!ctx)return;var w=48,h=48;canvas.width=w;canvas.height=h;ctx.clearRect(0,0,w,h);ctx.drawImage(sourceImg,0,0,w,h);var imgData=ctx.getImageData(0,0,w,h),data=imgData.data,totalLum=0,opaquePix=0,darkPix=0;for(var i=0;i<data.length;i+=4){var a=data[i+3];if(a>32){var r=data[i],g=data[i+1],b=data[i+2],lum=0.2126*r+0.7152*g+0.0722*b;totalLum+=lum;opaquePix++;if(lum<75)darkPix++}}if(opaquePix>=8){var avgLum=totalLum/opaquePix,darkRatio=darkPix/opaquePix;if(avgLum<95||darkRatio>0.6){imgEl.classList.add("is-dark-logo","vel-title-logo--dark");if(cardEl)cardEl.classList.add("has-dark-title-logo")}}}catch(err){if(!sourceImg._proxiedRetry&&imgEl.src&&!imgEl.src.startsWith("data:")&&!imgEl.src.includes("/api/proxy/")){var testImg=new Image();testImg.crossOrigin="anonymous";testImg._proxiedRetry=true;testImg.onload=function(){runAnalysis(testImg)};testImg.src="/api/proxy/image?url="+encodeURIComponent(imgEl.src)}}}if(imgEl.complete&&imgEl.naturalWidth){runAnalysis(imgEl)}else{imgEl.addEventListener("load",function(){runAnalysis(imgEl)},{once:true})}}
window.veloraDetectDarkLogo=veloraDetectDarkLogo;
...
async function openSectionContentDialog(row){
  ...
  try{
    ...
    if(row.package_id){
      try{
        var kind=row.content_type==="series"?"series":(row.content_type==="movies"?"vod":"live");
        var pRes=await fetch("/api/velora-db/admin/package-media-items?packageId="+encodeURIComponent(row.package_id)+"&kind="+encodeURIComponent(kind)+"&t="+Date.now(),{cache:"no-store"});
        if(pRes.ok){
          var pData=await pRes.json();
          if(pData&&Array.isArray(pData.items)){
            var customVisuals=new Map();
            if(Array.isArray(row.custom_entries)){
              row.custom_entries.forEach(function(cItem){
                var sId=String(cItem.streamId||cItem.stream_id||cItem.id||"");
                var sName=velNormStr(cItem.name||cItem.title||"");
                if(sId)customVisuals.set("id:"+sId, cItem);
                if(sName)customVisuals.set("name:"+sName, cItem);
              });
            }
            activeContentItems=pData.items.map(function(it){
              var rawId=it.stream_id||it.id;
              var poster=it.stream_icon||it.cover||"";
              var rawName=it.name||it.title||"";
              var cOverride=customVisuals.get("id:"+String(rawId))||customVisuals.get("name:"+velNormStr(rawName))||{};
              return {
                id:"home-cache:"+row.id+":"+rawId,
                name:rawName,
                thumbUrl:cOverride.thumbUrl||poster,
                backdropUrl:cOverride.backdropUrl||poster,
                title_logo:cOverride.title_logo||cOverride.titleLogo||"",
                horizontal_thumb:cOverride.horizontal_thumb||"",
                has_integrated_title:Boolean(cOverride.has_integrated_title),
                streamId:rawId,
      ...
      if(!activeContentItems.length&&typeof window.veloraGetHomeSectionContent==="function"){
    ...
    }else if(Array.isArray(row.custom_entries)&&row.custom_entries.length>0){
    ...
    if(isHoriz){
      if(!isHorizontalThumb&&titleLogoUrl&&titleLogoUrl!=="NONE"){
        ...
        function smartScale(){
          ...
          veloraDetectDarkLogo(logoImg,card);
        }
              ...
              if(!card.classList.contains("has-integrated-title")){
                ...
                if(!exLogo){
                  ...
                  function smartScaleDyn(){
                    ...
                    veloraDetectDarkLogo(dynLogo,card);
                  }
...
async function syncSectionContentFromPackage(){
  var syncBtn=document.getElementById("home-section-content-sync-pkg-btn"),
      statusEl=document.getElementById("home-section-content-status");
  if(!activeContentSection){
    if(statusEl)statusEl.textContent="Aucune section active sélectionnée.";
    return;
  }
  if(!activeContentSection.package_id){
    if(statusEl){
      statusEl.textContent="Cette section n'a pas de package lié (section personnalisée).";
  ...
  if(statusEl){
    statusEl.textContent="Chargement des éléments actuels du package depuis la base...";
    statusEl.style.color="#a78bfa";
  }
  try{
    var kind=activeContentSection.content_type==="series"?"series":(activeContentSection.content_type==="movies"?"vod":"live");
    var pRes=await fetch("/api/velora-db/admin/package-media-items?packageId="+encodeURIComponent(activeContentSection.package_id)+"&kind="+encodeURIComponent(kind)+"&t="+Date.now(),{cache:"no-store"});
    ...
    if(pData&&Array.isArray(pData.items)){
      activeContentItems=pData.items.map(function(it){
        ...
        var poster=it.stream_icon||it.cover||"";
        return {
          id:"home-cache:"+activeContentSection.id+":"+rawId,
          ...
          sourceId:it.source_id,
          containerExtension:it.container_extension||"",
          contentType:activeContentSection.content_type,
          packageId:activeContentSection.package_id
        };
      });
      var isHoriz=activeContentSection.card_orientation==="horizontal";
      if(isHoriz&&Array.isArray(activeContentItems)){
      ...
      renderContentDialogItems();
      if(statusEl){
        statusEl.textContent="✅ "+activeContentItems.length+" élément(s) synchronisé(s) depuis le package. Cliquez sur « Enregistrer le contenu » pour sauvegarder.";
        statusEl.style.color="#86efac";
      }
    }else{
      throw new Error("Aucun élément trouvé dans le package");
    }
  }catch(err){
    if(statusEl){
      statusEl.textContent="Erreur lors de la synchronisation : "+err.message;
      statusEl.style.color="#fca5a5";
    }
  }finally{
    if(syncBtn){
      syncBtn.disabled=false;
      syncBtn.textContent="🔄 Synchroniser depuis le package";
    }
  }
}
