(() => {
  ...
  const PACKAGE_RENDER_LIMIT=300;
  const NON_COUNTRIES=new Set(["adult","adulte"]);
  const st={sources:[],source:null,kind:"live",packages:[],loadedKinds:new Set(),loadingKinds:new Map(),allPackages:{live:[],vod:[],series:[]},countries:[],assigned:new Map(),packageOrders:new Map(),packageKinds:new Map(),visibleCountries:new Set(),activeCountry:null,pendingDeleteCountry:null,loaded:false,loadingCount:0}, $=id=>document.getElementById(id);
  ...
  function packageCountryNames(p){const names=[],packageName=p.name.trim().toLowerCase();for(const[countryId,packages]of st.assigned.entries())if(packages.some(x=>x.name.trim().toLowerCase()===packageName)){const c=st.countries.find(x=>x.id===countryId);if(c)names.push(c.name)}return names.sort((a,b)=>a.localeCompare(b,"fr"))}
  function drawPackages(){const q=($("mp-package-filter")?.value||"").trim(),searchWhole=$("mp-package-filter-whole")?.checked!==false,excluded=($("mp-package-exclude")?.value||"").split(/[,;\n]+/).map(x=>x.trim()).filter(Boolean),excludeWhole=$("mp-package-exclude-whole")?.checked!==false,rows=st.packages.filter(x=>(!q||textMatches(x.name,q,searchWhole))&&!excluded.some(term=>textMatches(x.name,term,excludeWhole))),rendered=rows.slice(0,PACKAGE_RENDER_LIMIT),filtered=q||excluded.length,bulk=rows.length>1?`<article class="manual-pays__package manual-pays__package--bulk" draggable="true" data-packages="${esc(JSON.stringify(rows.map(x=>x.key)))}"><strong>Glisser les ${rows.length} packages ${filtered?"filtrés":"affichés"}</strong><small>Déposez ce bloc sur un pays pour tout ajouter</small></article>`:"",more=rows.length>rendered.length?`<div class="manual-pays__empty">${rows.length-rendered.length} autres packages. Utilisez la recherche pour les afficher.</div>`:"";$("mp-package-list").innerHTML=rows.length?bulk+rendered.map(x=>{const assigned=packageCountryNames(x);return `<article class="manual-pays__package${assigned.length?" is-assigned":""}" draggable="true" data-package="${esc(x.key)}"><strong>${esc(x.name)}</strong><small>${x.label}${assigned.length?` · Ajouté à ${esc(assigned.join(", "))}`:""}</small></article>`}).join("")+more:`<div class="manual-pays__empty">${st.source?"Aucun package trouvé.":"Sélectionnez un stream."}</div>`}
  ...
  async function countries(){
    ...
    st.countries=admin;const [p,orders]=await Promise.all([sb("admin_packages","?select=id,country_id,name&order=name.asc"),packageOrderRows()]);st.assigned=new Map;st.packageOrders=new Map;st.packageKinds=new Map;for(const x of p){const a=st.assigned.get(x.country_id)||[];a.push(x);st.assigned.set(x.country_id,a)}for(const x of orders){const key=`${x.country_id}::${x.ui_tab}`,order=(Array.isArray(x.package_order)?x.package_order:[]).map(String);st.packageOrders.set(key,order);for(const id of order)st.packageKinds.set(id,String(x.ui_tab))}drawCountries();applyVisibility();if(st.activeCountry)drawDialog()
  }
  function applyVisibility(){window.__veloraVisibleCountries=new Set(st.visibleCountries);document.querySelectorAll("#country-select option,#home-country-select option,.vel-home-country-picker__option").forEach(node=>{const name=(node.textContent||"").trim().toLowerCase(),hidden=!st.visibleCountries.has(name)||NON_COUNTRIES.has(name);node.hidden=hidden;node.style.display=hidden?"none":""})}
  async function loadVisibility(){try{const rows=await canonicalRows();st.visibleCountries=new Set(rows.filter(x=>String(x.match_key).startsWith("__visible__:")).map(x=>String(x.display_name).trim().toLowerCase()));applyVisibility();window.dispatchEvent(new CustomEvent("velora-country-visibility-changed"))}catch(e){console.warn("[manual-pays] country visibility",e)}}
  async function toggleCountry(id){const c=st.countries.find(x=>x.id===id);if(!c)return;const visible=st.visibleCountries.has(c.name.toLowerCase());status(`${visible?"Masquage":"Affichage"} de ${c.name}...`);if(visible)await sb("canonical_countries",`?match_key=eq.${encodeURIComponent(`__visible__:${c.name.toLowerCase()}`)}`,{method:"DELETE",headers:{Prefer:"return=minimal"}});else await sb("canonical_countries","",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({match_key:`__visible__:${c.name.toLowerCase()}`,display_name:c.name,sort_order:999999})});await countries();window.dispatchEvent(new CustomEvent("velora-country-visibility-changed"));status(`${c.name} est maintenant ${visible?"masqué":"visible"} dans l'app.`)}
