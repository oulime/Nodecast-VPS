...
const sortedManualContentResolver = 'if(Ze(s)){const e=tG(s),t=G==="movies"?_.vodStreamsByCat:G==="series"?_.seriesStreamsByCat:_.streamsByCatAll,r=[],n=new Set;for(const i of t.values())for(const a of i){const o=G==="movies"||G==="series"?`${G==="movies"?"vod":G}:${a.nodecast_source_id??a.source_id??""}:${a.raw_stream_id??a.raw_series_id??a.stream_id??a.series_id??a.item_id??a.id}`:String(a.stream_id??a.series_id??a.item_id??a.id),l=String(a.stream_id??a.series_id??a.item_id??a.id);(e.has(o)||e.has(l))&&!n.has(o)&&(n.add(o),r.push(a))}return r.sort((i,a)=>Re(i.name).localeCompare(Re(a.name),"fr"))}';
const manualContentResolver = 'if(Ze(s)){const e=tG(s),t=G==="movies"?_.vodStreamsByCat:G==="series"?_.seriesStreamsByCat:_.streamsByCatAll,r=[],n=new Set;for(const i of t.values())for(const a of i){const o=G==="movies"||G==="series"?`${G==="movies"?"vod":G}:${a.nodecast_source_id??a.source_id??""}:${a.raw_stream_id??a.raw_series_id??a.stream_id??a.series_id??a.item_id??a.id}`:String(a.stream_id??a.series_id??a.item_id??a.id),l=String(a.stream_id??a.series_id??a.item_id??a.id);(e.has(o)||e.has(l))&&!n.has(o)&&(n.add(o),r.push(a))}return r}';
if (source.includes(sortedManualContentResolver)) {
    source = source.replaceAll(sortedManualContentResolver, manualContentResolver);
    changed = true;
}
