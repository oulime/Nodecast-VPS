...
class WatchPage {
    constructor(app) {
        ...
        this.isVodTranscode = false;
        this.currentSessionId = null;
        this.currentSourceUrl = null;
        this.currentStartAt = 0;
        this.durationSeconds = null;
        this.seekableTranscode = false;
        this.seekRestartInFlight = false;
    ...
    async startTranscodeSession(url, options = {}) {
        try {
            ...
            const startAt = Number.isFinite(Number(options.startAt))
                ? Math.max(0, Number(options.startAt))
                : Math.max(0, Number(this.resumeTime || 0));
            ...
            return {
                sessionId: null,
                playlistUrl: `/api/transcode?url=${encodeURIComponent(url)}`,
                status: 'fallback',
                startAt: 0,
                seekOffset: 0,
    ...
    getLogicalCurrentTime() {
        const mediaTime = this.video?.currentTime || 0;
        return this.isVodTranscode ? (this.currentStartAt + mediaTime) : mediaTime;
    }

    getDisplayedDuration() {
        if (this.isVodTranscode && Number.isFinite(this.durationSeconds) && this.durationSeconds > 0) {
            return this.durationSeconds;
        }
        return this.video?.duration || 0;
    }

    showLimitedSeekingMessage() {
        const msg = 'Seeking is limited for this source because the server is generating the video progressively.';
        this.updateTranscodeStatus('transcoding', msg);
    }

    async seekToTargetSeconds(targetSeconds) {
        if (this.seekRestartInFlight) return;
        if (!this.isVodTranscode || !this.seekableTranscode || !this.currentSourceUrl) {
            this.showLimitedSeekingMessage();
            return;
        }
        this.seekRestartInFlight = true;

        try {
            const clampedTarget = Math.max(0, Math.min(targetSeconds, this.durationSeconds || targetSeconds));
            const preparingLabel = this.formatTime(clampedTarget);
            this.video?.pause();
            this.showLoading();
            this.updateTranscodeStatus('transcoding', `Preparing from ${preparingLabel}...`);

            await this.stopTranscodeSession();

            const session = await this.startTranscodeSession(this.currentSourceUrl, {
                mode: 'vod',
                startAt: clampedTarget,
                videoMode: this.currentVideoMode,
                videoCodec: this.currentVideoCodec,
                audioCodec: this.currentAudioCodec,
                audioChannels: this.currentAudioChannels
            });

            this.currentStartAt = Number(session.startAt ?? session.seekOffset ?? clampedTarget) || 0;
            this.durationSeconds = session.durationSeconds ?? this.durationSeconds;
            this.seekableTranscode = session.seekable === true;
    ...
    async loadVideo(url) {
        ...
        this.currentAudioChannels = null;
        ...
        if (settings.autoTranscode) {
            ...
            try {
                ...
                if (info.needsTranscode || settings.upscaleEnabled) {
                    ...
                    this.currentAudioChannels = info.audioChannels;
                    ...
                    this.isVodTranscode = true;
                    this.currentStartAt = Number(session.startAt ?? session.seekOffset ?? 0) || 0;
                    this.durationSeconds = session.durationSeconds ?? null;
                    this.seekableTranscode = session.seekable === true;
        ...
        if (settings.forceTranscode) {
            ...
            this.currentVideoMode = 'copy';
            this.currentVideoCodec = videoCodec;
            const session = await this.startTranscodeSession(url, {
                mode: 'vod',
                videoMode: 'copy',
                videoCodec
    ...
    skip(seconds) {
        ...
        if (this.isVodTranscode && this.seekableTranscode && Number.isFinite(this.durationSeconds)) {
            const target = this.getLogicalCurrentTime() + seconds;
            this.seekToTargetSeconds(target);
            return;
        }
    ...
    seek(percent) {
        ...
        if (this.isVodTranscode && Number.isFinite(this.durationSeconds) && this.durationSeconds > 0) {
            const targetSeconds = ratio * this.durationSeconds;
            if (this.seekableTranscode) {
                this.seekToTargetSeconds(targetSeconds);
            } else {
                this.showLimitedSeekingMessage();
            }
            return;
    ...
    updateProgress() {
        ...
        const displayedCurrent = this.getLogicalCurrentTime();
        if (!displayedDuration || !Number.isFinite(displayedDuration)) return;

        const percent = (displayedCurrent / displayedDuration) * 100;
        this.progressSlider.value = percent;
        this.timeCurrent.textContent = this.formatTime(displayedCurrent);
        this.timeTotal.textContent = this.formatTime(displayedDuration);
    ...
    onMetadataLoaded() {
        ...
        // Handle non-session resumption only.
        if (this.resumeTime > 0 && this.video && !this.isVodTranscode) {
            const duration = this.video.duration;
        ...
        const displayedDuration = this.getDisplayedDuration();
        if (Number.isFinite(displayedDuration) && displayedDuration > 0) {
            this.timeTotal.textContent = this.formatTime(displayedDuration);
            this.timeCurrent.textContent = this.formatTime(this.getLogicalCurrentTime());
        }
    }
    ...
    async saveProgress() {
        ...
        const progress = Math.floor(this.getLogicalCurrentTime());
        const duration = Math.floor(this.getDisplayedDuration());
