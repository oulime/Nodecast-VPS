...
class ChannelList {
    ...
    async loadAllChannels() {
        ...
        try {
            ...
            await this.loadCombinedChannels();
    ...
    /**
     * Load channels from every enabled playlist source through one proxy request.
     */
    async loadCombinedChannels() {
        const sourceTypes = new Map(
            this.sources
                .filter(s => s.enabled && (s.type === 'xtream' || s.type === 'm3u'))
                .map(s => [String(s.id), s.type])
        );

        const [categories, streams] = await Promise.all([
            API.proxy.xtream.liveCategories('all'),
            API.proxy.xtream.liveStreams('all')
        ]);

        const categoryBySourceAndId = new Map();
        categories.forEach(cat => {
            const sourceId = cat.source_id;
            const sourceType = sourceTypes.get(String(sourceId)) || 'xtream';
            const rawCategoryId = cat.raw_category_id || cat.category_id;
            const categoryKey = `${sourceId}:${rawCategoryId}`;
            categoryBySourceAndId.set(categoryKey, cat);

            this.groups.push({
                id: `${sourceType}_${sourceId}_${rawCategoryId}`,
        ...
        streams.forEach(stream => {
            ...
            const sourceType = sourceTypes.get(String(sourceId)) || 'xtream';
            const rawStreamId = stream.raw_stream_id || stream.stream_id;
            const rawCategoryId = stream.raw_category_id || stream.category_id;
            const category = categoryBySourceAndId.get(`${sourceId}:${rawCategoryId}`);

            this.channels.push({
                id: `${sourceType}_${sourceId}_${rawStreamId}`,
                streamId: rawStreamId,
                globalStreamId: stream.global_stream_id || stream.stream_id,
                name: stream.name,
                tvgId: stream.epg_channel_id,
                tvgLogo: stream.stream_icon,
                url: stream.stream_url,
                groupId: `${sourceType}_${sourceId}_${rawCategoryId}`,
    ...
    async selectChannel(dataset) {
        ...
        if (channel.sourceType === 'xtream') {
            ...
            const result = channel.globalStreamId
                ? await API.proxy.xtream.getGlobalStreamUrl(channel.globalStreamId, 'live', streamFormat)
                : await API.proxy.xtream.getStreamUrl(channel.sourceId, channel.streamId, 'live', streamFormat);
