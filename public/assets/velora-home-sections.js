...
function stripChannelPrefixes(value){var original=String(value||"").trim(),name=original;for(var pass=0;pass<64;pass+=1){var prefix=homePrefixes.find(function(candidate){return candidate.length<=name.length&&name.slice(0,candidate.length).toLowerCase()===candidate.toLowerCase()});if(!prefix)break;name=name.slice(prefix.length).trim()}for(var pass=0;pass<64;pass+=1){var suffix=homeSuffixes.find(function(candidate){return candidate.length<=name.length&&name.slice(-candidate.length).toLowerCase()===candidate.toLowerCase()});if(!suffix)break;name=name.slice(0,-suffix.length).trim()}for(var p=0;p<5;p+=1){var next=name.replace(/^[\[\(]?[A-Z0-9\+\-\s]{1,12}[\]\)]\s*[-:]?\s*/i,"").replace(/^([0-9]+K|[0-9]+D|HD|FHD|UHD|4K|VF|VOSTFR|VO|FR|EN|ES|DE|MULTI|TRUEFRENCH|FRENCH)\s*[-:]?\s*/i,"").replace(/^[A-Z0-9]{1,8}-[A-Z0-9]{1,8}\s*[-:]?\s*/i,"").replace(/\s*([\[\(][A-Z0-9\+\-\s]{1,12}[\]\)]|\b(HD|FHD|UHD|4K|VF|VOSTFR|VO|FR|EN|ES|DE|MULTI|TRUEFRENCH|FRENCH)\b)$/i,"").trim();if(next===name)break;name=next}return name||original}
