...
const INITIAL_LIVE_SEGMENTS = readPositiveIntegerEnv('TRANSCODE_INITIAL_LIVE_SEGMENTS', 1);
...
async function cleanupStaleSessions() {
    ...
    await cleanupOrphanSessionDirs(now);
}

/**
 * Cleanup cache directories that are no longer tracked by the running server.
 *
 * This catches leftovers from crashes, forced exits, failed session creation,
 * or old cache folders without valid metadata. Active sessions are skipped.
 */
async function cleanupOrphanSessionDirs(now = Date.now()) {
    try {
    ...
    const dirents = await fs.readdir(CACHE_DIR, { withFileTypes: true });
    ...
    for (const dirent of dirents) {
        ...
        const sessionDir = path.join(CACHE_DIR, sessionId);
        let lastTouched = 0;

        try {
            const metaPath = path.join(sessionDir, 'session.json');
            const metadata = JSON.parse(await fs.readFile(metaPath, 'utf8'));
            lastTouched = Number(metadata.lastAccess || metadata.startTime || 0);
        } catch (_) {
            // Missing or invalid metadata: fall back to directory mtime.
        }
        ...
        if (!lastTouched) {
            try {
                ...
                console.warn(`[TranscodeSession] Failed to stat orphan cache ${sessionId}:`, err.message);
                continue;
            }
        }

        if (now - lastTouched > ORPHAN_SESSION_MAX_AGE_MS) {
            try {
                await fs.rm(sessionDir, { recursive: true, force: true });
                console.log(`[TranscodeSession] Removed orphan cache folder ${sessionId}`);
            } catch (err) {
                console.error(`[TranscodeSession] Failed to remove orphan cache ${sessionId}:`, err.message);
            }
        }
    }
}
...
function startCleanupInterval() {
    if (!cleanupInterval) {
        cleanupStaleSessions().catch(err => {
            console.error('[TranscodeSession] Initial cleanup failed:', err.message);
        });
...
module.exports = {
    ...
    LIVE_PLAYLIST_READY_TIMEOUT_MS,
