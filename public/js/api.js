...
const API = {
    ...
    proxy: {
        ...
        xtream: {
            ...
            getGlobalStreamUrl: (streamId, type = 'live', container = 'm3u8') =>
                API.request('GET', `/proxy/xtream/stream/${encodeURIComponent(streamId)}/${type}?container=${container}`)
        },
    ...
    // Transcode sessions
    transcode: {
        createSession: (data) => API.request('POST', '/transcode/session', data),
        stopSession: (sessionId) => API.request('DELETE', `/transcode/${sessionId}`)
    },
